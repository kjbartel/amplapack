...
template <enum class option::ordering storage_type, typename value_type>
void larft(const concurrency::accelerator_view& /*av*/, enum class option::direction /*direct*/, enum class option::storage storev, concurrency::array_view<value_type,2>& v, concurrency::array_view<value_type,1>& tau, concurrency::array_view<value_type,2>& t)
{
    static_assert(storage_type == option::ordering::column_major, "hybrid functionality requires column major ordering");

    const int n = (storev == option::storage::column ? get_rows<storage_type>(v) : get_cols<storage_type>(v));
    const int k = (storev == option::storage::column ? get_cols<storage_type>(v) : get_rows<storage_type>(v));
    ...
    for (int i = 0; i < k; i += block_size)
    {
        ...
        if (i+ib < n)
        {
            ...
            // form the triangular factor (t) of the block reflector
            {
                ...
                host::larft<storage_type>(av, option::direction::forward, option::storage::column, a_sub, tau_sub, t_sub);
            }
            ...
            // w = t' * v' <==> w' = v * t 
            {
                ...
                ampblas::gemm(av, AmpblasNoTrans, AmpblasNoTrans, value_type(1), a_sub, b_sub, value_type(), c_sub);
            }
            ...
            // w = w' * a2
            {
                ...
                ampblas::gemm(av, AmpblasTrans, AmpblasNoTrans, value_type(1), a_sub, b_sub, value_type(), c_sub);
            }
...
void geqrf(concurrency::accelerator_view& av, int m, int n, value_type* a, int lda, value_type* tau)
{
    ...
    // forwarding function
    _detail::geqrf<option::ordering::column_major>(av, accl_view_a, host_view_tau);
